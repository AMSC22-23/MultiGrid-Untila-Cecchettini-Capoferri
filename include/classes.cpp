...
class Iteration{
    ...
    public:
        ...
        virtual double Res_calc(std::vector<double> &sol, std::vector<double> &res) const = 0;
...
class Gauss_Siedel_iteration : public Iteration<Vector>{
    ...
    public:
        ...
        double Res_calc(std::vector<double> &sol, std::vector<double> &res) const override {   
            double res_norm = 0;
            for(size_t i = 0; i < m_A.rows(); i++){
                size_t index = m_A.mask(i);
                size_t temp = 0;
                for(const auto &id : m_A.nonZerosInRow(i)){
                    temp += m_A.coeffRef(i,id) * sol[m_A.mask(id)];
                }
                res_norm += (b[index] - temp) * (b[index] - temp);
                res[index] = b[index] - temp;
            }
